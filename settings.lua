...
function InterruptTrack:InitSettings()
	...
	AddCheckbox("KICKROTATION", true, function()
		InterruptTrack:SendHello(true)
		InterruptTrack:RefreshAnnouncer()
		InterruptTrack:UpdateBars()
	end)

	AddCheckbox("ROTATIONSKIPHEALER", true, function() InterruptTrack:UpdateBars() end)
	if InterruptTrack:GetWoWBuild() ~= "RETAIL" then AddCheckbox("ANNOUNCEKICKCHAT", false) end
...
eventFrame:SetScript("OnEvent", function(self, event, ...)
	if event == "PLAYER_LOGIN" then
		...
		InterruptTrack:SetVersion(132219, "0.1.4")
